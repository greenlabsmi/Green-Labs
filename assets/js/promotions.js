(() => {
  ...
  function updateBanner(banner) {
    ...
    ["headline", "products", "offer"].forEach(part => {
      document.querySelectorAll(`[data-promo-banner-part="${part}"]`).forEach(el => el.textContent = banner[part] || "");
    });
  }
  ...
  function updateBrandHeroCopy() {
    ...
    if (sub) sub.textContent = "Independent. Award-winning. Different by design. Green Labs brings together exceptional cannabis, Michigan’s most sought-after brands, and a team that actually knows the products we sell.";
  ...
  function updateHero(hero) {
    ...
    if (link) { link.href = hero.href || "#deli"; link.setAttribute("aria-label", hero.ariaLabel || "Explore the Green Labs Deli"); link.removeAttribute("data-open-deals"); link.removeAttribute("data-open-shop"); link.querySelector(".promo-hero-shop-cta")?.remove(); }
    if (image) { image.style.backgroundImage = `url("${hero.image}")`; image.style.backgroundPosition = hero.position || "center"; image.style.backgroundSize = hero.size || "cover"; image.style.backgroundRepeat = "no-repeat"; image.setAttribute("aria-label", hero.ariaLabel || "Green Labs Deli hero"); }
  }
  ...
  function updatePopup(popup) {
    ...
    if (!popup || popup.enabled === false) { if (wrap) wrap.hidden = true; return; }
  ...
  function closePopup() {
    const wrap = document.getElementById("weeklyPromoPopup"), video = document.getElementById("weeklyPromoVideo");
    if (!wrap) return; wrap.classList.remove("is-open"); wrap.classList.add("is-collapsed"); wrap.querySelector(".weekly-promo__tab")?.setAttribute("aria-expanded", "false"); if (video && !video.hidden) video.pause();
  }
  function expandPopup() {
    const wrap = document.getElementById("weeklyPromoPopup"); if (!wrap) return; wrap.hidden = false; wrap.classList.add("is-open"); wrap.classList.remove("is-collapsed"); wrap.querySelector(".weekly-promo__tab")?.setAttribute("aria-expanded", "true");
  }
  function initPopupControls() {
    const wrap = document.getElementById("weeklyPromoPopup"), link = document.getElementById("weeklyPromoLink"); if (!wrap) return;
    wrap.querySelector(".weekly-promo__tab")?.addEventListener("click", () => wrap.classList.contains("is-open") ? closePopup() : expandPopup());
    wrap.querySelectorAll("[data-close-weekly-promo]").forEach(el => el.addEventListener("click", closePopup)); link?.addEventListener("click", closePopup);
    document.addEventListener("keydown", e => { if (e.key === "Escape" && !wrap.hidden) closePopup(); });
  }
  function showPopup(popup, name) {
    if (!popup || popup.enabled === false) return; const wrap = document.getElementById("weeklyPromoPopup"); if (!wrap) return; const key = storageKey(popup, name);
    if (getStored(key)) { wrap.hidden = false; wrap.classList.add("is-collapsed"); return; }
    setTimeout(() => { expandPopup(); setStored(key, String(Date.now())); }, Number.isFinite(popup.delay) && popup.delay >= 0 ? popup.delay : 10000);
  }
  function init() {
    const name = activeName(), campaign = PROMOTIONS[name] || PROMOTIONS.default; window.GreenLabsPromotion = {name, campaign, promotions:PROMOTIONS};
    updateBrandHeroCopy(); updateBanner(campaign.banner); updateHero(BRAND_HERO); updatePopup(campaign.popup); initPopupControls(); showPopup(campaign.popup, name);
