(() => {
  ...
  function updateHero(hero) { const link = document.getElementById("promoHeroLink"), image = document.getElementById("promoHeroImage"); if (link) { link.href = hero.href || "#deli"; link.setAttribute("aria-label", hero.ariaLabel || "Explore the Green Labs Deli"); link.removeAttribute("data-open-deals"); link.removeAttribute("data-open-shop"); link.querySelector(".promo-hero-shop-cta")?.remove(); } if (image) { image.style.backgroundImage = `url("${hero.image}")`; image.style.backgroundPosition = hero.position || "center"; image.style.backgroundSize = hero.size || "cover"; image.style.backgroundRepeat = "no-repeat"; image.setAttribute("aria-label", hero.ariaLabel || "Green Labs Deli hero"); } }
  ...
  function init() { const name = activeName(), campaign = PROMOTIONS[name] || PROMOTIONS.default; window.GreenLabsPromotion = {name, campaign, promotions:PROMOTIONS}; updateBrandHeroCopy(); updateBanner(campaign.banner); updateHero(BRAND_HERO); updatePopup(campaign.popup); initPopupControls(); showPopup(campaign.popup, name); console.info(`[Green Labs Promotions] Active campaign: ${name}`); }
