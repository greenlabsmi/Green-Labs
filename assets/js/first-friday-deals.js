(() => {
  ...
  onReady(() => {
    ...
    const vendorCard = document.querySelector("#event-details .ff-feature-card--vendor");
    const musicCard = document.querySelector("#event-details .ff-feature-card--music");

    // True North is no longer an in-store vendor. Keep the weekend deal only.
    const intro = [...document.querySelectorAll("#event-details p")].find((p) => p.textContent.includes("Batch and True North joining the party"));
    if (intro) {
      intro.textContent = "Toledo’s own Decent Folk takes the stage, with our friends from Batch joining the party. Plus, deals from Rove, Glacier, True North, and more, local makers, giant Jenga, cornhole, and plenty more.";
    }

    // Repurpose the old vendor card as City Soda.
    if (vendorCard) {
      vendorCard.classList.add("ff-feature-card--city-soda");
    ...
    // Keep shared artwork and Decent Folk details.
    ...
    // Deals before Lemon Wookie. True North remains B2G1 all weekend, with no vendor timing.
    if (cardGrid && wookieCard && !document.getElementById("ff-first-friday-deal-hub")) {
      ...
      hub.innerHTML = `
        <div class="ff-deal-hub__head">
          ...
          <p>Deals run Friday unless otherwise noted. ROVE and the True North B2G1 continue through Sept. 7.</p>
        </div>
        <div class="ff-deal-hub__art">
          <img id="ff-deals-art" src="/assets/img/promotions/first-friday-deals-sept-2026.png" alt="First Friday deal lineup at Green Labs" loading="eager" decoding="async">
        </div>
        <div class="ff-deal-hub__copy">
          ...
          <div class="ff-deal-hub__all-deals" aria-label="All First Friday offers">
            ...
            <div><strong>BUY 2 GET 1 FREE</strong><span>True North <small>Sept. 4–7</small></span></div>
    ...
    style.textContent = `
      .ff-first-friday-deal-hub{grid-column:1/-1;margin:1.1rem 0 .35rem;padding:clamp(1rem,2vw,1.35rem);border:1px solid rgba(15,77,58,.16);border-radius:22px;background:#f7f3e9;box-shadow:0 18px 42px rgba(15,39,30,.08);overflow:hidden}
      ...
      /* Shared banner uses natural image height so no artwork is cropped. */
      ...
      .ff-music-feature__art img{display:block;width:100%;height:auto!important;aspect-ratio:auto!important;object-fit:contain!important}

      /* Matching accent line on both info boxes. */
      .ff-music-feature__copy,.ff-feature-card--city-soda{position:relative;padding:20px 22px 22px!important;border-top:4px solid transparent!important;border-image:linear-gradient(90deg,#0b7d5a,#79c9a7,#dc951e) 1!important}
      .ff-music-feature__copy{border-right:1px solid var(--ff-border)}
      .ff-feature-card--city-soda{grid-column:auto!important;margin-top:0!important}
    ...
    if (musicCard && vendorCard) {
      const copy = musicCard.querySelector(".ff-music-feature__copy");
      if (copy && vendorCard.parentElement === cardGrid) {
        vendorCard.remove();
        musicCard.appendChild(vendorCard);
      }
    }
  });
})();